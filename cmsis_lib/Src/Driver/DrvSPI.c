...
//*****************************************************************************
//
//! \brief Set 2-bit transfer mode.When enable 2-bit transfer mode,the Tx_NUM must 
//!  be configure as 0x00 (one transaction in one transfer.)
//! \param eSpiPort : Specify the SPI port
...
//*****************************************************************************
//
//! \brief Set Dertermine to transfer data with LSB first or MSB first. 
//! \param eSpiPort : Specify the SPI port
//! \param eEndian  : Specify LSB first or MSB first
//!                   (eDRVSPI_LSB_FIRST / eDRVSPI_MSB_FIRST)
//!
//! \return None.  
//
//*****************************************************************************
...
//! \param eSpiPort : Specify the SPI port
//! \param eOption: the options of Byte Reorder function and Byte Suspend function. 
...
//!  which indicate 2~17 SPI clock cycles. In FIFO mode, it could be 2~15 and 0.
//!  0 indicates the maximum suspend interval; 2 indicates the minimum suspend
//!  interval. Please refer to TRM for the actual suspend interval. 
//!
//! \note Only for master mode. The suspend cycle setting is shared with burst 
//!  mode, byte suspend function and FIFO mode. 
//!
//! \return E_DRVSPI_ERR_SUSPEND_INTERVAL: The suspend interval setting is out
//!  of range. E_SUCCESS: Success.  
//
//*****************************************************************************
...
//!  eDRVSPI_ACTIVE_HIGH_RISING : Slave select pin is active high in
//!  level-trigger mode; or rising-edge trigger in edge-trigger mode. 
//!
//! \return None.  
//
//*****************************************************************************
...
//!
//! \param eSpiPort : Specify the SPI port
//! \param eSlaveSel : Select the slave select pins which will be used. It could
//!  be eDRVSPI_NONE, eDRVSPI_SS0, eDRVSPI_SS1 and eDRVSPI_SS0_SS1. 
//!
//! \return None.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Set the slave select pins. Only for master mode. 
//!
//! \param eSpiPort : Specify the SPI port
//! \param eSlaveSel : In automatic slave select operation, to use this
//!  parameter to select the slave select pins which will be used. The specified
//!  slave select pins will be controlled by hardware. In manual slave select
//!  operation, the specified slave select pins will be set to active state. It
//!  could be eDRVSPI_NONE, eDRVSPI_SS0, eDRVSPI_SS1 or eDRVSPI_SS0_SS1. 
//!
//! \return None.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Set the specified slave select pins to inactive state. Only for
//!  master mode. 
//!
//! \param eSpiPort : Specify the SPI port
//! \param eSlaveSel : Specify slave select pins It could be eDRVSPI_NONE,
//!  eDRVSPI_SS0, eDRVSPI_SS1 or eDRVSPI_SS0_SS1. 
//!
//! \return None.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Configure the SPI clock. Only for master mode.If the DIV_ONE bit is
//!  set to 1, executing this function is unmeaningful.
//!
//! \param eSpiPort : Specify the SPI port
...
//*****************************************************************************
//
//! \brief Returns the SPI interrupt flag 
//!
//! \param eSpiPort : Specify the SPI port 
//!
//! \return 0: SPI interrupt doesn't occur 1: SPI interrupt occurs  
//
//*****************************************************************************
...
//!
//! \return TRUE:  The data stored in pu32Data is valid. FALSE: The data stored
//!  in pu32Data is invalid.  
//
//*****************************************************************************
...
//!  bus. 
//!
//! \return TRUE:  The data stored in pu32Data has been transferred. FALSE: The
//!  SPI is in busy. The data stored in pu32Data has not been transferred.  
//
//*****************************************************************************
...
//!
//! \return TRUE: The data stored in pu32Buf is valid. FALSE: The data stored in
//!  pu32Buf is invalid.  
//
//*****************************************************************************
...
//!  bus. 
//!
//! \return TRUE: The data stored in pu32Buf has been transferred. FALSE: The
//!  SPI is in busy. The data stored in pu32Buf has not been transferred.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Configure the DMA settings. 
//!
//! \param eSpiPort :  Specify the SPI port
//!      eDmaMode   :  Specify the DMA mode.
//!                    eDRVSPI_TX_DMA -- DMA-Transmitting
//!                    eDRVSPI_RX_DMA -- DMA-Receiving 
//!    bEnable         TRUE  -- Enable DMA; 
//!                    FALSE -- Disable DMA.  
//!
//! \return None.  
//
...
//*****************************************************************************
//
//! \brief Enable/disable FIFO mode. If the caller enables FIFO mode, configure
//! the setting of suspend cycle. Only the chips with the part number NUC1x0xxxCx,
//! ex: NUC140VE3CN, can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//! \param bEnable : Enable (TRUE) / Disable (FALSE)
//! \param i32Interval : In FIFO mode, it could be 2~15 and 0. 0 indicates the
//!  maximum suspend interval; 2 indicates the minimum suspend interval. Please
//!  refer to NUC122 TRM for the actual suspend interval. 
//!
//! \return E_DRVSPI_ERR_SUSPEND_INTERVAL: The suspend interval setting is out
//!  of range. E_SUCCESS: Success.  
//
...
//*****************************************************************************
//
//! \brief Check the status of the Rx buffer of the specified SPI port. Only the
//! chips with the part number NUC1x0xxxCx, ex: NUC140VE3CN, can support this 
//! function.
//!
//! \param eSpiPort : Specify the SPI port 
//!
//! \return TRUE:  Rx buffer is empty. FALSE: Rx buffer is not empty.  
//
//*****************************************************************************
...
//! \param eSpiPort : Specify the SPI port 
//!
//! \return TRUE:  Rx buffer is full. FALSE: Rx buffer is not full.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Check the status of the Tx buffer of the specified SPI port.Only the 
//!  chips with the part number NUC1x0xxxCx, ex: NUC140VE3CN, can support this 
//!  function. 
//!
//! \param eSpiPort : Specify the SPI port 
//!
//! \return TRUE:  Tx buffer is empty. FALSE: Tx buffer is not empty.  
//
//*****************************************************************************
...
//! \param eSpiPort : Specify the SPI port 
//!
//! \return TRUE:  Tx buffer is full. FALSE: Tx buffer is not full.  
//
//*****************************************************************************
...
//*****************************************************************************
//
//! \brief Clear the Rx FIFO.Only the chips with the part number NUC1x0xxxCx, 
//! ex:NUC140VE3CN,can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//!
...
//*****************************************************************************
//
//! \brief Clear the Tx FIFO.Only the chips with the part number NUC1x0xxxCx, 
//! ex:NUC140VE3CN,can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//!
...
//*****************************************************************************
//
//! \brief enable the DIV_ONE feature. The SPI clock rate is determined by the
//! setting of SPI clock divider.Only the chips with the part number NUC1x0xxxCx, 
//! ex:NUC140VE3CN,can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//!
...
//*****************************************************************************
//
//! \brief enable the SPI 3-wire function.Only the chips with the part number 
//! NUC1x0xxxCx,ex:NUC140VE3CN,can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//!
//! \return None
...
//*****************************************************************************
//
//! \brief Disable the SPI 3-wire function.Only the chips with the part number 
//! NUC1x0xxxCx,ex:NUC140VE3CN,can support this function.
//!
//! \param eSpiPort : Specify the SPI port
//!
...
//*****************************************************************************
//
//! \brief Abort transfer when using 3-wire SPI. Specify the SPI port If using 
//! 3-wire SPI as slave, when slave start interrupt status is set but transfer 
//! done flag doesn't be set over a reasonable time, use this function to abort 
//! this transfer.Only the chips with the part number NUC1x0xxxCx,ex:NUC140VE3CN,
...
//*****************************************************************************
//
//! \brief Enable the 3-wire SPI start interruptof the specified SPI port and 
//! install the callback function.Only the chips with the part number NUC1x0xxxCx,
//! ex: NUC140VE3CN, can support this function.
//!
//! \param eSpiPort : Specify the SPI port
...
//*****************************************************************************
//
//! \brief Disable the 3-wire SPI start interrupt.Only the chips with the part
//!  number NUC1x0xxxCx, ex: NUC140VE3CN, can support this function.
//!
//! \param eSpiPort : Specify the SPI port 
//!
//! \return None
...
//*****************************************************************************
//
//! \brief Return the 3-wire SPI start interrupt status.Only the chips with the
//! part number NUC1x0xxxCx, ex: NUC140VE3CN, can support this function.
//!
//! \param eSpiPort : Specify the SPI port 
//!
...
//*****************************************************************************
//
//! \brief Clear the 3-wire SPI start interrupt status.Only the chips with the
//! part number NUC1x0xxxCx, ex: NUC140VE3CN, can support this function.
//!
//! \param eSpiPort : Specify the SPI port 
//!
...
//*****************************************************************************
//
//! \brief Get the version number of NUC122 SPI driver. 
//!
//! \param None 
//!
